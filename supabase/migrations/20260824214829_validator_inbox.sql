-- RLS-scoped metrics and browsing for the validator task inbox.
create function public.get_upload_assignment_metrics(p_upload_id uuid)
returns table (
  task_count bigint,
...
as $$
  select
    ...
    (
      ...
      from public.validation_alerts alert
      where alert.upload_id = p_upload_id
        and alert.category = 'orthography'::public.alert_category
    ) as orthography_count;
$$;

create function public.browse_review_tasks(
  p_upload_id uuid,
...
revoke all on function public.get_upload_assignment_metrics(uuid) from public, anon, authenticated, service_role;
grant execute on function public.get_upload_assignment_metrics(uuid) to authenticated, service_role;

revoke all on function public.browse_review_tasks(uuid, public.review_status, text, text, text, integer, integer)
from public, anon, authenticated, service_role;
grant execute on function public.browse_review_tasks(uuid, public.review_status, text, text, text, integer, integer)
to authenticated, service_role;

comment on function public.get_upload_assignment_metrics(uuid) is
  'Returns upload metrics through the caller RLS scope, so validators only count their own assignment.';

comment on function public.browse_review_tasks(uuid, public.review_status, text, text, text, integer, integer) is
  'Returns an RLS-scoped task page with stable global rule ordering and the filtered result count.';
