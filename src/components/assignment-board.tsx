...
export function AssignmentBoard({ uploadId, initialUploadVersion, initialBlocks, expectedBlockCount, validators }: { uploadId: string; initialUploadVersion: number; initialBlocks: Block[]; expectedBlockCount: number; validators: Validator[] }) {
  ...
  async function propose() {
    ...
    if (returnedBlockCount !== expectedBlockCount || assignments.length !== expectedBlockCount || blocks.some((block) => !snapshotByBlock.has(block.id))) {
      setError('No se cargaron todos los bloques recalculados. Actualiza la página antes de publicar.');
      router.refresh();
      return;
    }
