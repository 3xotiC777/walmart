...
export default async function TaskPage({ params }: { params: Promise<{ taskId: string }> }) {
  ...
  return <><header className="page-heading"><div><p className="overline">TAREA · FILA {source?.excel_row}</p><h1>{source?.description || 'Registro sin descripción'}</h1><p>Row-Id <span className="mono">{source?.row_id || '—'}</span> · Código <span className="mono">{source?.barcode || '—'}</span> · Id_Dn W <span className="mono">{source?.id_dn_w || '—'}</span></p></div><Link className="button button-secondary" href="/workspace/tareas" prefetch={false}>← Volver a la bandeja</Link></header>{task.is_related_only && alerts.length === 0 ? <RelatedEditor taskId={task.id} version={task.version} fields={source?.field_values ?? {}} headers={headers} status={task.status} role={viewer.role}/> : <TaskReview taskId={task.id} blockId={block!.id} blockVersion={block!.version} blockAssignedTo={block!.assigned_to} role={viewer.role} alerts={alerts} resolutions={resolutions} invoices={invoices}/>}</>;
}
