...
export default async function TaskPage({ params }: { params: Promise<{ taskId: string }> }) {
  ...
  const invoicePromise = source?.id_dn_w
    ? supabase.from('invoice_links').select('external_url').eq('upload_id', task.upload_id).eq('id_dn_w', source.id_dn_w).not('external_url', 'is', null).range(0, 19)
    : Promise.resolve({ data: [] as Array<{ external_url: string | null }> });
  const uploadPromise = supabase
    .from('uploads')
    .select('source_headers')
    .eq('id', task.upload_id)
    .maybeSingle();
  const [{ data: invoiceRows }, { data: upload }] = await Promise.all([
    invoicePromise,
    uploadPromise,
  ]);
  ...
  return <><header className="page-heading"><div><p className="overline">TAREA · FILA {source?.excel_row}</p><h1>{source?.description || 'Registro sin descripción'}</h1><p>Row-Id <span className="mono">{source?.row_id || '—'}</span> · Código <span className="mono">{source?.barcode || '—'}</span> · Id_Dn W <span className="mono">{source?.id_dn_w || '—'}</span></p></div><Link className="button button-secondary" href="/workspace/tareas" prefetch={false} transitionTypes={['workspace-back']}>← Volver a la bandeja</Link></header>{task.is_related_only && alerts.length === 0 ? <RelatedEditor taskId={task.id} version={task.version} fields={source?.field_values ?? {}} headers={headers} status={task.status} role={viewer.role}/> : <TaskReview taskId={task.id} blockId={block!.id} blockVersion={block!.version} blockAssignedTo={block!.assigned_to} role={viewer.role} alerts={alerts} resolutions={resolutions} invoices={invoices}/>}</>;
}
